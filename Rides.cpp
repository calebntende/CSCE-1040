...
std::unordered_map<int, Driver*> available_drivers;
for (int i = 0; i < drivers.size(); i++) {
    if (drivers[i].get_availability()) {
        available_drivers[i] = &drivers[i];
    }
}

// Create an unordered_map of available passengers
std::unordered_map<int, Passenger*> available_passengers;
for (int i = 0; i < passengers.size(); i++) {
    if (passengers[i].get_Passenger_handicapped()) {
        available_passengers[i] = &passengers[i];
    }
}

// Find a driver that meets the criteria
for (auto& driver_pair : available_drivers) 
{
    Driver& driver = *driver_pair.second;
    if (driver.get_capacity() >= party_size &&
        driver.get_pets() == ride_pets &&
        driver.get_rating() >= ride_rating) 
    {
        // Find a passenger that meets the criteria
        for (auto& passenger_pair : available_passengers) {
            Passenger& passenger = *passenger_pair.second;
            if (driver.get_handicapped() == passenger.get_Passenger_handicapped()) {
